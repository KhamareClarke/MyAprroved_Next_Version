...
export default function TradespersonProfile() {
  ...
  useEffect(() => {
    const fetchTradesperson = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/tradespeople/${params.id}`);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Tradesperson Not Found</h1>
          <p className="text-gray-600 mb-6">{error || 'The tradesperson you are looking for does not exist.'}</p>
          <Button asChild>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex space-x-8">
              <Link href="/find-tradespeople" className="text-blue-700 font-medium">
                Find Tradespeople
              </Link>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center space-x-3">
                        <h1 className="text-2xl font-bold text-gray-900">{tradesperson.name}</h1>
                        {tradesperson.verified && (
                          <Shield className="w-6 h-6 text-green-500" />
                        )}
                      </div>
                      <div className="text-right">
                        <div className="text-xl font-bold text-blue-700">{tradesperson.hourlyRate}</div>
                        <div className="text-sm text-gray-600">Response: {tradesperson.responseTime}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-4 mb-4">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="ml-1 font-semibold">{tradesperson.rating || 'No rating'}</span>
                        <span className="text-gray-600">({tradesperson.reviews} reviews)</span>
                      </div>
                    </div>
                    
                    <p className="text-gray-700 mb-4">{tradesperson.description}</p>
            ...
            <Card>
              <CardContent className="p-6">
                <h2 className="text-xl font-bold mb-4">Reviews ({tradesperson.reviews})</h2>
                  ...
                  <div className="space-y-4">
                    {tradesperson.reviewsData.map((review) => (
                      <div key={review.id} className="border-b border-gray-200 pb-4 last:border-b-0">
                        ...
                        <span className="text-xs text-gray-500 capitalize">{review.reviewerType}</span>
                      </div>
                    ))}
                  </div>
                ) : (
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{tradesperson.reviews}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Average Rating</span>
                    <span className="font-semibold">{tradesperson.rating || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Years Experience</span>
                    <span className="font-semibold">{tradesperson.yearsExperience}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Response Time</span>
                    <span className="font-semibold">{tradesperson.responseTime}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
