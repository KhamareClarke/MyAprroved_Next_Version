...
import { CheckCircle, XCircle, Star, Plus, LogOut, Clock, MessageCircle, ChevronDown, Wrench, Phone, Mail, User } from 'lucide-react';
...
export default function ClientDashboardPage() {
  ...
  const loadChatUnreadCount = async (userId: string) => {
    try {
      const response = await fetch(`/api/chat/unread-count?userId=${userId}&userType=client`);
  ...
  const loadJobApplications = async (jobId: string) => {
    try {
      const { data, error } = await supabase
        ...
        .select(`*, tradespeople (id, first_name, last_name, trade, years_experience, hourly_rate, phone, email)`) 
        .eq('job_id', jobId)
  ...
  const handleAssignJob = (job: any) => {
    if (job.assigned_by === 'admin') {
      setError('This job has already been assigned by admin. You cannot reassign it.');
      return;
    }
  ...
  const submitJobCompletion = async (ratings: { [tradespersonId: string]: { rating: number, review: string } }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const ratingPromises = Object.entries(ratings).map(([tradespersonId, ratingData]) => 
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-blue-600">
                {jobs.filter(job => job.assigned_tradesperson_id && !job.is_completed).length}
              </div>
                  ...
                  <TableBody>
                    {jobs.map((job) => (
                      <TableRow key={job.id}>
                        ...
                        <TableCell>
                          <div>
                            <div className="font-medium">£{job.budget || 'Not specified'}</div>
                          ...
                          <TableRow key={application.id}>
                            <TableCell>
                              <div>
                                ...
                                <div className="text-sm text-gray-500">{application.tradespeople.years_experience} years exp.</div>
                                <div className="text-xs text-gray-400">£{application.tradespeople.hourly_rate}/hr</div>
                                <div className="text-xs text-gray-400">{application.tradespeople.phone}</div>
                              </div>
                            </TableCell>
                            <TableCell>
                              <div>
                            ...
                            <TableCell>
                              {application.status === 'pending' && !selectedJobForAssignment.assigned_tradesperson_id && selectedJobForAssignment.application_status === 'open' && (
                                <div className="space-y-1">
                                  <Button
                                    onClick={() => handleApproveQuotation(application.id, 'approve')}
                                    disabled={assigning}
                              ...
                              {selectedJobForAssignment.assigned_tradesperson_id === application.tradesperson_id && (
                                <Badge variant="default" className="w-full justify-center">
                                  ✓ Assigned
                                </Badge>
                              )}
                  ...
                  <div className="bg-green-50 border-2 border-green-200 text-green-800 p-3 rounded-lg text-center">
                    ...
                    <div className="text-sm">
                      This job has been assigned to {
                        jobApplications.find(app => app.tradesperson_id === selectedJobForAssignment.assigned_tradesperson_id)?.tradespeople.first_name
                      } {jobApplications.find(app => app.tradesperson_id === selectedJobForAssignment.assigned_tradesperson_id)?.tradespeople.last_name}
                    </div>
                    <div className="text-xs mt-1">No further assignments are allowed.</div>
                  </div>
                )}
