  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { success: false, error: `Failed to fetch tradesperson: ${error.message}` },
        { status: 500 }
      );
    }
    ...
    // Transform the data
    const fullName = `${tradesperson.first_name || ''} ${tradesperson.last_name || ''}`.trim();
    ...
    const mockData = {
      ...
      description: `Professional ${tradesperson.trade?.toLowerCase() || 'tradesperson'} with ${tradesperson.years_experience || 'several'} years of experience in ${tradesperson.city || 'the area'}.`,
    ...
    const hourlyRate = tradesperson.hourly_rate 
      ...
      : `£${Math.floor(Math.random() * 30) + 30}-${Math.floor(Math.random() * 30) + 60}/hr`;
    ...
    const transformedTradesperson = {
      ...
      location: `${tradesperson.city || 'Unknown'}, ${tradesperson.postcode || 'Unknown'}`,
      distance: mockData.distance,
      image: tradesperson.profile_picture_url || null,
      initials: initials,
      verified: tradesperson.is_verified || false,
      yearsExperience: tradesperson.years_experience || Math.floor(Math.random() * 15) + 3,
