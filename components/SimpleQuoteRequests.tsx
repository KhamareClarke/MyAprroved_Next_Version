...
export default function SimpleQuoteRequests() {
  ...
  const fetchQuoteRequests = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/admin/quote-requests?ts=${Date.now()}`, {
        cache: 'no-store'
      } as RequestInit);
  ...
  const handleApproveReject = async (quoteRequestId: string, action: 'approve' | 'reject') => {
    try {
      ...
      if (data.success) {
        ...
        setQuoteRequests(prevRequests => prevRequests.filter(request => request.id !== quoteRequestId));
  ...
  const pendingRequests = quoteRequests.filter(r => r.status === 'pending_admin_approval');
      ...
      {pendingRequests.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-sm text-gray-400 mt-2">All quote requests have been processed</p>
          </CardContent>
        </Card>
      ) : (
          ...
          {pendingRequests.map((request) => (
            <Card key={request.id} className="border-l-4 border-l-blue-500">
              <CardHeader>
                ...
                <div className="mt-1 text-xs text-gray-400">ID: {request.id}</div>
              </CardHeader>
