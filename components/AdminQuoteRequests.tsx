...
export default function AdminQuoteRequests() {
  ...
  console.log('AdminQuoteRequests: Component rendered, loading:', loading, 'quoteRequests:', quoteRequests);
  ...
  const fetchQuoteRequests = async () => {
    try {
      ...
      if (data.success) {
        console.log('AdminQuoteRequests: Setting quote requests:', data.quoteRequests);
  ...
  const handleApproveReject = async (quoteRequestId: string, action: 'approve' | 'reject') => {
    try {
      setLoading(true); // Show loading state
      const response = await fetch('/api/admin/approve-quote', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        setSuccess(`Quote request ${action}d successfully! Email notifications have been sent.`);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-yellow-100 p-4 rounded mb-4">
        <p className="text-yellow-800">Debug: AdminQuoteRequests component loaded. Loading: {loading ? 'Yes' : 'No'}, Requests: {quoteRequests.length}</p>
      </div>
